...
class ReportSynthesizer:
    ...
    def build_full_report(
        ...
        for ev in (evidence_snippets or []):
            ...
            if p_id:
                evidence_counts[p_id] = evidence_counts.get(p_id, 0) + 1

        citation_list = []
        for doc in documents:
            final_sc = doc.metadata.get("final_score", 0.0) if doc.metadata else 0.0
            if isinstance(final_sc, (int, float)):
                ui_relevance = 1.0 if final_sc >= 1.0 else round(float(final_sc), 2)
            else:
                ui_relevance = 0.0

            ev_count = evidence_counts.get(doc.id, 0)
